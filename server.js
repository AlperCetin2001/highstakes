...
io.on('connection', (socket) => {
    ...
    socket.on('startGame', () => {
        ...
        addLog(room, `Oyun Başladı! Sıra: ${room.players[room.turnIndex].nickname}`);
        startTurnTimer(room);
        broadcastGameState(roomId);
    });
    ...
    socket.on('playCard', ({ cardIndex, chosenColor }) => {
        ...
        if (isValid) {
            ...
            // --- UNO CEZA KONTROLÜ ---
            // Eğer oyuncu kart attıktan sonra elinde 1 kart kaldıysa ve önceden UNO butonuna basmamışsa
            ...
            // Kart sayısı 1 değilse UNO durumunu temizle
    ...
    socket.on('callUno', () => {
        ...
        // Elinde 2 kart varken (biri atılacak) UNO diyebilir.
    ...
    socket.on('challengeDecision', ({ decision }) => {
        ...
        if (socket.id !== victimId) return; // Sadece mağdur cevap verebilir
        ...
        if (decision === 'accept') {
            ...
            // Blöf kontrolü: Attacker'ın elinde eski renkten (Joker hariç) kart var mı?
            const hasColor = attacker.hand.some(c => c.color === oldColor && c.color !== 'black');
            if (hasColor) {
                addLog(room, `⚖️ YAKALANDI! ${attacker.nickname} blöf yaptı! (Ceza: 4 Kart)`);
                drawCards(room, attacker, 4);
                // Sıra mağdurda kalır, avantaj ona geçer
                // Ancak discard pile'da +4 duruyor, renk değişti. Mağdur oynamalı.
            } else {
...
function handleCardEffect(room, card, player, oldColorForChallenge) {
    ...
    if (card.value === 'skip') { 
        ...
        addLog(room, `${next.nickname} +2 yedi ve sırası geçti!`);
        skipNext = true; // DÜZELTME: Kart çeken oynayamaz
    }
